  ...
  form.parse(req, async (err, fields, files) => {
    ...
    try {
      ...
      formData.append("inputFile", fileData, {
        ...
        contentType: singleFile.mimetype || "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
      });
      ...
      res.setHeader("Content-Disposition", `attachment; filename="${singleFile.originalFilename.replace(/\.(docx|doc)$/i, ".pdf")}"`);
      return res.send(Buffer.from(response.data));
    } catch (error) {
      console.error("Word Conversion Error:", error.message);
      let errorMsg = "Failed to convert Word document.";
